...
from utils_causal_language_modeling import tokenize_function, group_texts, CLMDataset, collate_fn
...
def load_model_and_tokenizer(args):
    ...
    if args["load_weights"]:
        ...
        model = AutoModelForCausalLM.from_config(AutoConfig.from_pretrained(args["model_path"]))
...
def load_and_prepare_dataset(args, tokenizer):
    ...
    lm_datasets = raw_ds.map(
        lambda x: tokenize_function(x, tokenizer), batched=True, remove_columns=raw_ds.column_names, num_proc=256
    ...
    train_ds = CLMDataset(split["train"].map(
        lambda x: group_texts(x, block_size=max_length, stride=max_length//2, tokenizer=tokenizer, bos_token_id=args["bos_token_id"]),  batched=True, num_proc=256
    ), tokenizer=tokenizer)
