...
from src.training.contrastive_learning.triplet_loss.collator import collate_fn
from src.training.contrastive_learning.triplet_loss.dataset import (
    TripletLossDataset,
    tokenize_function,
)
from src.training.contrastive_learning.triplet_loss.loss import forward_loss
...
def load_and_prepare_dataset(args, tokenizer):
    ...
    if len(tokenized_dataset) < 2:
        raise ValueError("Triplet loss training requires at least 2 examples.")
    ...
    train_dataset = TripletLossDataset(split["train"])
    eval_dataset = TripletLossDataset(split["test"])
    print(train_dataset)
    print(eval_dataset)
