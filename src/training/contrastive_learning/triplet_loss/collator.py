...
def collate_fn(batch, tokenizer):
    ...
    negative_input_ids = [example["negative_input_ids"] for example in batch]
    negative_attention_mask = [example["negative_attention_mask"] for example in batch]
    ...
    return {
        ...
        "negative_input_ids": pad_sequence(
            negative_input_ids, batch_first=True, padding_value=tokenizer.pad_token_id
        ),
        "negative_attention_mask": pad_sequence(
            negative_attention_mask, batch_first=True, padding_value=0
        ),
    }
