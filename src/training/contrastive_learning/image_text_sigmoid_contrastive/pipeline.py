...
from src.training.contrastive_learning.image_text_sigmoid_contrastive.collator import (
    collate_fn,
)
from src.training.contrastive_learning.image_text_sigmoid_contrastive.dataset import (
    ImageTextSigmoidContrastiveDataset,
)
from src.training.contrastive_learning.image_text_sigmoid_contrastive.loss import (
    forward_loss,
)
...
def load_and_prepare_dataset(args, tokenizer, image_processor):
    ...
    if len(raw_dataset) < 2:
        raise ValueError(
            "Image-text sigmoid contrastive training requires at least 2 examples."
        )
    ...
    train_dataset = ImageTextSigmoidContrastiveDataset(split["train"])
    eval_dataset = ImageTextSigmoidContrastiveDataset(split["test"])
    print(train_dataset)
    print(eval_dataset)
