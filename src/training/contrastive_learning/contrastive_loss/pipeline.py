...
from src.training.contrastive_learning.contrastive_loss.collator import collate_fn
from src.training.contrastive_learning.contrastive_loss.dataset import (
    ContrastiveLossDataset,
    tokenize_function,
)
from src.training.contrastive_learning.contrastive_loss.loss import forward_loss
...
def load_and_prepare_dataset(args, tokenizer):
    ...
    tokenized_dataset = raw_dataset.map(
        ...
        remove_columns=[
            col for col in raw_dataset.column_names if col != "label"
        ],
        num_proc=256,
    )
    ...
    if len(tokenized_dataset) < 2:
        raise ValueError("Contrastive loss training requires at least 2 examples.")
    ...
    train_dataset = ContrastiveLossDataset(split["train"])
    eval_dataset = ContrastiveLossDataset(split["test"])
    print(train_dataset)
    print(eval_dataset)
...
def load_model(args):
    ...
    model = load_encoder_model(args["model_path"], args["device"], quantization_config=qcfg)
    model = apply_peft_to_model(model, args, task_type="FEATURE_EXTRACTION")
    print("Model loaded...")
    return model
...
def train(args, model, tokenizer, train_loader, eval_loader, optimizer, scaler, scheduler):
    ...
    for epoch in range(args["num_epochs"]):
        ...
        for step, batch in enumerate(loop):
            ...
            loss = forward_loss(model, batch, margin=margin)
            scaled_loss = loss / ga
        ...
        with torch.no_grad():
            for batch in eval_loader:
                batch = move_batch_to_device(batch, device)
                eval_loss = forward_loss(model, batch, margin=margin)
